...
/// <summary>
/// Integration tests using real SegFormer-b0 ONNX model.
/// These tests require the model to be downloaded first:
///   pwsh scripts/download-test-models.ps1
/// </summary>
public class SegmentationIntegrationTests : IDisposable
{
    private const string ModelPath = "models/segformer/model.onnx";
    private const int NumADE20KClasses = 150;
    ...
    public void ModelMetadata_HasExpectedInput()
    {
        ...
        // Input shape: [batch, 3, H, W] — dynamic axes report -1
        Assert.Equal(4, meta.InputShapes[0].Length);

        Assert.Single(meta.OutputNames);
        Assert.Equal("logits", meta.OutputNames[0]);

        // Output shape has 4 dimensions: [batch, numClasses, H, W]
    ...
    public void Segment_ReturnsValidSegmentationMask()
    {
        ...
        // Mask should be non-null with valid ClassIds
        Assert.NotNull(mask);
        ...
        Assert.Equal(512, mask.Height);
        Assert.Equal(512 * 512, mask.ClassIds.Length);

        // All class IDs should be valid ADE20K classes [0, 149]
        foreach (var classId in mask.ClassIds)
        {
            Assert.InRange(classId, 0, NumADE20KClasses - 1);
        }
    }
    ...
    public void Segment_GetClassAt_ReturnsValidClassIds()
    {
        ...
        int[] testPositions = [0, 100, 255, 400, 511];
    ...
    public void Segment_WithoutResize_ReturnsModelNativeResolution()
    {
        ...
        // Without resize, output should be model's native spatial resolution (512/4 = 128)
        Assert.Equal(128, mask.Width);
        Assert.Equal(128, mask.Height);
